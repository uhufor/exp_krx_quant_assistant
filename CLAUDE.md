...
`tests/fixtures/sample_ohlcv.csv`: 5종목 × 252거래일(2024년) 합성 데이터. `FixtureAdapter`가 이 파일을 읽어 네트워크 없이 전체 파이프라인 테스트.

`tests/fixtures/sample_ohlcv_10y.csv`: **실제 KRX 수정주가** 5종목(005930·000660·035420·051910·006400) × 2458거래일(2015-01-02~2024-12-30, 12290행). `--data-source fixture_10y` 또는 `FixtureAdapter(fixture_path=FIXTURE_10Y_PATH)`로 사용한다. 합성 1년치로는 드러나지 않는 것 — 워크포워드 **롤링창의 효과**(짧은 구간에서는 확장창과 차이가 없다), 국면 전환(2018 −24% · 2020 +45% · 2022 −29% · 2024 −32%), 모멘텀 팩터의 273일 워밍업 — 을 오프라인으로 확인하기 위한 픽스처다. pykrx 수집 시 두 가지를 보정했다(`tests/unit/data/test_fixture_10y.py`가 형상 고정): ① **액면분할 거래정지일**은 pykrx가 `open/high/low`를 0으로 주므로 `O=H=L=C`로 채웠다(6행) ② **수정주가 반올림 오차**로 `close`가 `high`를 1원 넘는 행이 있어 `high=max(OHLC)`/`low=min(OHLC)`로 넓혔다(39행) — 보정하지 않으면 ATR·볼린저처럼 고저가를 쓰는 팩터가 조용히 오염된다. **OHLCV만 10년치이고 밸류에이션·재무제표 픽스처는 2024년 1년치뿐**이라, 펀더멘털 팩터를 쓰는 전략은 그 바깥 구간에서 NaN으로 자연 탈락한다.
