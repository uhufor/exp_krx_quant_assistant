...
| | 설명 |
|---|---|
| **전략 설계** | 팩터 32종을 산술 조합(Formula)하고 조건(Rule)으로 엮어 전략을 만듭니다. 코드 없이 JSON 또는 GUI 트리 편집기로 |
...
```
src/quant_krx/
  factors/       팩터 32종 순수 계산 (가격·기술 7 + 밸류에이션 11 + 재무제표 14)
  formula/       팩터 산술 조합 정의
...
| **Fixture** | 합성 데이터 — 네트워크 없이 오프라인 검증용 |
